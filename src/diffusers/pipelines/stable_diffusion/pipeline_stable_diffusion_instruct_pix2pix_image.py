...
#         condition_image = prepare_image(new_image)
...
# def preprocess(image):
#     if isinstance(image, torch.Tensor):
#         # Ensure tensor is in float16
#         return image.to(dtype=torch.float16)
#     elif isinstance(image, PIL.Image.Image):
#         image = [image]
...
#         image = image.transpose(0, 3, 1, 2)
#         image = 2.0 * image - 1.0
#         image = torch.from_numpy(image).to(dtype=torch.float16)  # Convert to float16
#     elif isinstance(image[0], torch.Tensor):
#         image = torch.cat(image, dim=0).to(dtype=torch.float16)  # Ensure tensor is in float16
#     return image
...
#         image = np.concatenate([np.array(i.convert("RGB"))[None, :] for i in image], axis=0)
#         image = image.transpose(0, 3, 1, 2)
#         image = torch.from_numpy(image).to(dtype=torch.float16) / 127.5 - 1.0  # Convert to float16
...
#             # Concatenate the unconditional and image embeddings into a single batch
#             image_embeddings = torch.cat([image_embeddings, negative_prompt_embeds, negative_prompt_embeds], dim=0)
...
#         new_image=None,
#     ):
#         # 0. Check inputs - Removed prompt checks
#         if image is None:
#             raise ValueError("`image` input cannot be undefined.")
...
#                 # Concatenate latents and image latents
#                 scaled_latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
#                 scaled_latent_model_input = torch.cat([scaled_latent_model_input, image_latents2], dim=1)
...
class StableDiffusionInstructPix2PixImagePipeline(StableDiffusionInstructPix2PixPipeline):
    ...
    ## WE TRIED CHANGING THIS FUNCTION FROM CHINESE VERSION TO OPENAI, WE FAILED NOT FINAL 
    def _encode_image(self, image, device, num_images_per_prompt, do_classifier_free_guidance):
        dtype = next(self.image_encoder.parameters()).dtype
        ...
        # Extract the last hidden state (image embeddings)
        image_embeddings = output.last_hidden_state.to(device=device, dtype=dtype)

        # For the negative prompt embeddings, use the pooler output or any other required method
        negative_prompt_embeds = output.pooler_output.to(device=device, dtype=dtype)
        ...
        if do_classifier_free_guidance:
            # Ensure negative_prompt_embeds has the same batch size as image_embeddings
            negative_prompt_embeds = negative_prompt_embeds.unsqueeze(1).repeat(1, seq_len, 1)
            
            # Duplicate for each generation per prompt
            negative_prompt_embeds = negative_prompt_embeds.repeat(num_images_per_prompt, 1, 1)

            # Concatenate the unconditional and image embeddings
            image_embeddings = torch.cat([image_embeddings, negative_prompt_embeds, negative_prompt_embeds], dim=0)
    ...
    def __call__(
        ...
        # )
        image_embeddings = self._encode_image(
            new_image, device, num_images_per_prompt, do_classifier_free_guidance
        )
